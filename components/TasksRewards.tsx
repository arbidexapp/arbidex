...
import { grantSocialTaskPoints, getMyRank, getTxFromSupabase, getGrantedTaskKeys } from '@/lib/supabase-db';
...
const s = { bg: 'bg-orange-50', text: 'text-orange-600', border: 'border-orange-200' };

// ── Swap Card ─────────────────────────────────────────────────────────────────
function SwapTaskCard({ tradeCount, grantedKeys }: { tradeCount: number; grantedKeys: string[] }) {
  return (
    <div className={`bg-white rounded-2xl border ${s.border} shadow-sm overflow-hidden`}>
      ...
      <div className="px-4 py-3 space-y-2">
        {SWAP_MILESTONES.map((m) => {
          const done = grantedKeys.includes(`milestone_swap_${m.count}`);
          ...
          return (
            <div key={m.count} className={`rounded-xl p-2 border transition-all ${done ? 'bg-green-50 border-green-200' : 'bg-gray-50 border-gray-100'}`}>
              <div className="flex items-center justify-between mb-1">
                <span className="text-xs text-gray-700 font-medium">{done ? '✅' : '🔒'} {m.count} Swaps</span>
                {!done && <span className="text-xs text-gray-400 font-medium">{progress} / {m.count}</span>}
              </div>
              {!done && (
                <div className="w-full bg-gray-200 rounded-full h-1.5">
                  <div className="bg-gradient-to-r from-orange-400 to-amber-400 h-1.5 rounded-full transition-all"
...
// ── Send Card ─────────────────────────────────────────────────────────────────
function SendTaskCard({ sendCount, grantedKeys }: { sendCount: number; grantedKeys: string[] }) {
  return (
    <div className={`bg-white rounded-2xl border ${s.border} shadow-sm overflow-hidden`}>
      <div className={`${s.bg} px-4 py-3 flex items-center gap-2`}>
        <span className="text-xl">📤</span>
      ...
      <div className="px-4 py-3 space-y-2">
        {SEND_MILESTONES.map((m) => {
          const done = grantedKeys.includes(`milestone_send_${m.count}`);
          const progress = Math.min(sendCount, m.count);
          return (
            <div key={m.count} className={`rounded-xl p-2 border transition-all ${done ? 'bg-green-50 border-green-200' : 'bg-gray-50 border-gray-100'}`}>
              <div className="flex items-center justify-between mb-1">
                <span className="text-xs text-gray-700 font-medium">{done ? '✅' : '🔒'} {m.count} Sends</span>
                {!done && <span className="text-xs text-gray-400 font-medium">{progress} / {m.count}</span>}
              </div>
              {!done && (
                <div className="w-full bg-gray-200 rounded-full h-1.5">
...
// ── Social Card ───────────────────────────────────────────────────────────────
function SocialTaskCard({ wallet, grantedKeys, onGrant }: {
  wallet: string;
  grantedKeys: string[];
  onGrant: () => void;
}) {
  const [loading, setLoading] = useState(false);
  const done = grantedKeys.includes('social_x_follow');
      ...
      <div className="px-4 py-3 border-b border-gray-100">
        <div className={`rounded-xl p-2 border transition-all ${done ? 'bg-green-50 border-green-200' : 'bg-gray-50 border-gray-100'}`}>
          <div className="flex items-center justify-between gap-2">
            <span className="text-xs text-gray-700 font-medium">{done ? '✅' : '🔒'} Follow on X</span>
...
export function TasksRewards() {
  ...
  const load = async (addr: string) => {
    try {
      const [rank, txs, keys] = await Promise.all([
        getMyRank(addr),
        getTxFromSupabase(addr),
        getGrantedTaskKeys(addr),
      ]);
  ...
  useEffect(() => {
    if (!address) return;
    load(address);
    window.addEventListener('aggrex_tx_update', () => load(address));
    return () => window.removeEventListener('aggrex_tx_update', () => load(address));
  }, [address]);
  ...
  return (
    <div className="space-y-4 max-w-3xl mx-auto">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <SwapTaskCard tradeCount={tradeCount} grantedKeys={grantedKeys} />
        <SendTaskCard sendCount={sendCount} grantedKeys={grantedKeys} />
        <SocialTaskCard wallet={address} grantedKeys={grantedKeys} onGrant={() => load(address)} />
      </div>
