...
function Code({ children }: { children: React.ReactNode }) {
  return <code className="bg-orange-50 border border-orange-100 text-orange-700 rounded px-1.5 py-0.5 text-xs font-mono">{children}</code>;
}
function H2({ id, children }: { id: string; children: React.ReactNode }) {
  return <h2 id={id} className="text-2xl font-bold text-gray-900 mt-10 mb-3 scroll-mt-24 border-b border-orange-100 pb-2">{children}</h2>;
}
function H3({ id, children }: { id?: string; children: React.ReactNode }) {
  return <h3 id={id} className="text-lg font-bold text-gray-800 mt-6 mb-2 scroll-mt-24">{children}</h3>;
}
        ...
        <tbody className="divide-y divide-orange-50 bg-white">
          {rows.map((row, i) => (
            <tr key={i} className="hover:bg-orange-50/40 transition-colors">
              {row.map((cell, j) => <td key={j} className="px-4 py-2.5 text-gray-700 font-mono text-xs">{cell}</td>)}
            </tr>
          ))}
        </tbody>
      </table>
  ...
  <div>
    ...
    <ul className="list-none space-y-2 mb-4">
      {[
        ...
        ['🏆','Best route suggestion','— automatically selects the DEX that gives the highest output.'],
        ['🔒','Zero intermediary contract risk','— transactions go directly through the relevant DEX\'s own audited router.'],
      ].map(([icon,title,desc]) => (
        <li key={String(title)} className="flex gap-2 text-sm text-gray-600">
    ...
    <H2 id="overview-diagram">Product Overview</H2>
    <MermaidDiagram chart={`
flowchart LR
    Wallet(["Connect Wallet"]) --> Nav{Navigation}
    Nav --> Swap["Swap"]
    Nav --> Send["Send"]
    Nav --> Leaderboard["Leaderboard"]
    Nav --> Tasks["Tasks & Rewards"]
    Nav --> History["History"]
    Nav --> Portfolio["Portfolio"]
    Swap -. "earns points" .-> Tasks
    Send -. "earns points" .-> Tasks
    Tasks -. "feeds ranking" .-> Leaderboard
    Swap -. "logs record" .-> History
    Send -. "logs record" .-> History
`} />
  </div>
);
  ...
  <div>
    ...
    <ul className="list-none space-y-2 mb-4 text-sm text-gray-600">
      {[
        ...
        ['Action button','Shows "Enter an amount" when empty, becomes "Swap" once route is ready.'],
      ].map(([label,desc]) => (
    ...
    <ol className="list-decimal list-inside space-y-2 text-sm text-gray-600 mb-2 pl-2">
      ...
      <li>Multiple fee tiers scanned for Uniswap and PancakeSwap: <Code>0.01%</Code> <Code>0.05%</Code> <Code>0.3%</Code> <Code>1%</Code>.</li>
      <li>Aerodrome checked on both classic (stable/volatile) and Slipstream (concentrated liquidity) pools.</li>
      <li>Route with the highest <Code>amountOut</Code> is selected as the "best route."</li>
    </ol>

    <MermaidDiagram chart={`
flowchart TD
    A["User enters amount + token pair"] --> B["Query Uniswap V3\\n(all fee tiers)"]
    A --> C["Query PancakeSwap V3\\n(all fee tiers)"]
    A --> D["Query Aerodrome\\n(classic + Slipstream)"]
    B --> E["Compare all amountOut results"]
    C --> E
    D --> E
    E --> F["Show best route to user"]
`} />
...
sequenceDiagram
    ...
    participant R as Selected DEX Router
    U->>R: approve(token, amount) [if needed]
...
const tasksDocs = (
  <div>
    ...
    <MermaidDiagram chart={`
flowchart LR
    Swap["Swap\\n+200 pts each"] --> Total["Total Points"]
    Send["Send\\n+100 pts each"] --> Total
    Milestones["10/25/50 Milestones\\none-time bonus"] --> Total
    Social["Social Tasks\\ne.g. Follow on X"] --> Total
    Total --> LB["Leaderboard Rank"]
`} />
...
`} />
    ...
    <ol className="list-decimal list-inside space-y-2 text-sm text-gray-600 mb-4 pl-2">
      ...
      <li>Sends the approved transaction <strong>directly</strong> to the selected DEX's own audited router contract.</li>
    </ol>
    ...
    <p className="text-gray-600 text-sm leading-relaxed mb-3">
      All quotes are fetched in parallel and compared. The route with the highest <Code>amountOut</Code> wins.
    </p>
...
sequenceDiagram
    ...
    participant AeroQ as Aerodrome Quoter(s)
    App->>MC: batched eth_call (all quotes)
    MC->>UniQ: quoteExactInputSingle (per fee tier)
    MC->>PanQ: quoteExactInputSingle (per fee tier)
    MC->>AeroQ: getAmountsOut (stable + volatile + Slipstream)
    UniQ-->>MC: amountOut
    PanQ-->>MC: amountOut
    AeroQ-->>MC: amountOut
    MC-->>App: all results in one response
    App->>App: pick highest amountOut
`} />
    ...
    <p className="text-xs text-gray-500 mb-4">Fee tiers: <Code>0.01%</Code> <Code>0.05%</Code> <Code>0.3%</Code> <Code>1%</Code></p>
    ...
    <Danger>All addresses listed here belong to <strong>Base Mainnet</strong>. Base Sepolia (testnet) addresses must never be used in production.</Danger>
  </div>
);
  ...
  <div>
    ...
    <ul className="list-none space-y-3 mb-4">
      {[
        ...
        <li key={String(title)} className="flex gap-3 bg-orange-50 border border-orange-100 rounded-xl p-3 text-sm">
  ...
  <div>
    ...
    <div className="space-y-3">
      {[
        ...
        ['Do points have any redeemable value?','Whether points will be redeemable for a token or other reward will be announced separately.'],
        ['Why did my transaction fail?','Most common: insufficient balance, slippage too low, or deadline expired. Check History and inspect on BaseScan.'],
      ].map(([q,a]) => (
        <details key={String(q)} className="group bg-white border border-orange-100 rounded-2xl overflow-hidden">
...
const PAGES: DocPage[] = [
  ...
  { id:'swap', label:'Swap', group:'Guides',
    sections:[{id:'swap-ui',title:'UI Components'},{id:'route-finding',title:'Route-Finding'},{id:'execution-flow',title:'Execution Flow'}],
    content: swapDocs },
  { id:'send', label:'Send', group:'Guides',
    sections:[{id:'send-ui',title:'UI Components'},{id:'send-flow',title:'Flow'}],
    content: sendDocs },
  { id:'leaderboard', label:'Leaderboard', group:'Guides',
    sections:[{id:'lb-personal',title:'Personal Summary'},{id:'lb-table',title:'Top Traders'}],
    content: leaderboardDocs },
  { id:'tasks', label:'Tasks & Rewards', group:'Guides',
    sections:[{id:'tasks-swap',title:'Swap Tasks'},{id:'tasks-send',title:'Send Tasks'},{id:'tasks-social',title:'Social Tasks'}],
    content: tasksDocs },
  { id:'architecture', label:'Architecture', group:'Reference',
    sections:[{id:'arch-flow',title:'How it Works'},{id:'arch-comparison',title:'Comparison'},{id:'routing-engine',title:'Routing Engine'},{id:'supported-dexs',title:'Supported DEXs'}],
    content: architectureDocs },
  { id:'security', label:'Security', group:'Reference',
    sections:[{id:'sec-principles',title:'Core Principles'},{id:'sec-user',title:'User Responsibilities'}],
    content: securityDocs },
  { id:'faq', label:'FAQ', group:'Reference', sections:[], content: faqDocs },
];
            ...
            <div key={group} className="mb-5">
              ...
              {PAGES.filter((p) => p.group === group).map((p) => (
                <button key={p.id} onClick={() => goTo(p.id)}
                  className={`w-full text-left px-3 py-2 rounded-lg text-sm font-medium transition-all mb-0.5 ${activePage === p.id ? 'bg-orange-100 text-orange-700 font-semibold' : 'text-gray-600 hover:bg-orange-50 hover:text-orange-600'}`}>
                  {p.label}
                </button>
              ))}
            </div>
          ...
          <div className="mt-12 pt-6 border-t border-orange-100 flex items-center justify-between">
            {(() => {
              ...
              return (
                <>
                  {prev ? (
                    <button onClick={() => goTo(prev.id)} className="flex items-center gap-2 text-sm text-orange-600 hover:text-orange-700 font-semibold">← {prev.label}</button>
                  ) : <div />}
                  {next ? (
                    <button onClick={() => goTo(next.id)} className="flex items-center gap-2 text-sm text-orange-600 hover:text-orange-700 font-semibold">{next.label} →</button>
                  ) : <div />}
                </>
              );
            })()}
        ...
        {page.sections.length > 0 && (
          <aside className="hidden xl:block w-44 shrink-0 sticky top-20 h-fit">
            ...
            <ul className="space-y-1.5">
              {page.sections.map((s) => (
                <li key={s.id}>
                  <button onClick={() => document.getElementById(s.id)?.scrollIntoView({ behavior:'smooth', block:'start' })}
